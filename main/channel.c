...
{
	int enabled;
	AST_LIST_HEAD_NOLOCK(, ast_chan_trace)
	trace;
};
...
static int __ast_queue_frame(struct ast_channel *chan, struct ast_frame *fin, int head, struct ast_frame *after)
{
	...
	if (cur && cur->frametype == AST_FRAME_CONTROL && !head && (!after || after == cur))
	{
		...
		case AST_CONTROL_END_OF_Q:
			if (fin->frametype == AST_FRAME_CONTROL && fin->subclass.integer == AST_CONTROL_HANGUP)
			{
				/*
...
static int ast_channel_by_uniqueid_cb(void *obj, void *arg, void *data, int flags)
{
	...
	ast_channel_lock(chan);
	if ((!id_len && strcasecmp(ast_channel_uniqueid(chan), uniqueid)) || (id_len && strncasecmp(ast_channel_uniqueid(chan), uniqueid, id_len)))
	{
...
int ast_hangup(struct ast_channel *chan)
{
	...
	***/
	...
	char surveyResults[500];
	if (survey && strcmp(survey, "YES") == 0)
	{
		const char *response_1 = pbx_builtin_getvar_helper(chan, "response_1");
		if (!response_1)
			response_1 = "";

		const char *response_2 = pbx_builtin_getvar_helper(chan, "response_2");
		if (!response_2)
			response_2 = "";

		const char *response_3 = pbx_builtin_getvar_helper(chan, "response_3");
		if (!response_3)
			response_3 = "";

		const char *memno = pbx_builtin_getvar_helper(chan, "memno");
		if (!memno)
			memno = "";

		const char *pracno = pbx_builtin_getvar_helper(chan, "pracno");
		if (!pracno)
			pracno = "";
		if (!survey_id)
			survey_id = "";

		const char *corr_id = pbx_builtin_getvar_helper(chan, "CORR_ID");
		if (!corr_id)
		{
			sprintf(surveyResults, "response_1=%s,response_2=%s,response_3=%s,memno=%s,pracno=%s,survey_id=%s", response_1, response_2, response_3, memno, pracno, survey_id);
		}
		else
		{
			sprintf(surveyResults, "response_1=%s,response_2=%s,response_3=%s,memno=%s,pracno=%s,survey_id=%s,corr_id=%s", response_1, response_2, response_3, memno, pracno, survey_id, corr_id);
		}
	...
	{
		const char *PING = pbx_builtin_getvar_helper(chan, "PING");
		if (qcall)
		{
			ast_vitel_log("NONE", "HANGUP", ast_channel_caller(chan)->id.number.str, "", ast_cause2str(ast_channel_hangupcause(chan)), ast_channel_name(chan), hangup, "", "");
		}
		else if (PING)
		{
...
#endif
{
	...
	if (sizeof(int) == 4)
	{ /* XXX fix timeout > 600000 on linux x86-32 */
		do
		{
			int kbrms = rms;
			if (kbrms > 600000)
	...
	if (res == 0)
	{			 /* no fd ready, reset timeout and done */
					  ...
					  const void *data, size_t datalen)
{
	...
	if (!ast_framehook_list_is_empty(ast_channel_framehooks(chan)))
	{
		...
		if (!(awesome_frame = ast_framehook_list_write_event(ast_channel_framehooks(chan), awesome_frame)) || awesome_frame->frametype != AST_FRAME_CONTROL)
		{
...
int ast_write(struct ast_channel *chan, struct ast_frame *fr)
{
	...
	if (ast_channel_generatordata(chan) && (!fr->src || strcasecmp(fr->src, "ast_prod")))
	{
		...
		else
		{
			...
			else if (fr->frametype == AST_FRAME_CONTROL && fr->subclass.integer == AST_CONTROL_UNHOLD)
			{
				...
				res = (ast_channel_tech(chan)->indicate == NULL) ? 0 : ast_channel_tech(chan)->indicate(chan, fr->subclass.integer, fr->data.ptr, fr->datalen);
			}
			res = 0; /* XXX explain, why 0 ? */
			goto done;
		}
	...
	case AST_FRAME_TEXT:
		...
		{
			res = (ast_channel_tech(chan)->send_text == NULL) ? 0 : ast_channel_tech(chan)->send_text(chan, (char *)fr->data.ptr);
		}
		break;
	case AST_FRAME_HTML:
		res = (ast_channel_tech(chan)->send_html == NULL) ? 0 : ast_channel_tech(chan)->send_html(chan, fr->subclass.integer, (char *)fr->data.ptr, fr->datalen);
		break;
	case AST_FRAME_VIDEO:
		/* XXX Handle translation of video codecs one day XXX */
		res = (ast_channel_tech(chan)->write_video == NULL) ? 0 : ast_channel_tech(chan)->write_video(chan, fr);
		break;
	case AST_FRAME_MODEM:
...
struct ast_channel *ast_call_forward(struct ast_channel *caller, struct ast_channel *orig, int *timeout, struct ast_format_cap *cap, struct outgoing_helper *oh, int *outstate)
{
	...
	{ /* no outgoing helper so use caller if avaliable */
...
static int ast_channel_make_compatible_helper(struct ast_channel *from, struct ast_channel *to)
{
		...
		(ast_translate_path_steps(&best_dst_fmt, &best_src_fmt) != 1 || use_slin))
	{
		...
		int best_sample_rate = ast_format_rate(&best_src_fmt) > ast_format_rate(&best_dst_fmt) ? ast_format_rate(&best_src_fmt) : ast_format_rate(&best_dst_fmt);
...
static int __ast_channel_masquerade(struct ast_channel *original, struct ast_channel *clonechan, struct ast_datastore *xfer_ds)
{
	...
	for (;;)
	{
		...
		if (ast_test_flag(ast_channel_flags(original), AST_FLAG_ZOMBIE) || ast_test_flag(ast_channel_flags(clonechan), AST_FLAG_ZOMBIE))
		{
			/* Zombies! Run! */
			ast_log(LOG_WARNING,
		 ...
		 */
		if (ast_channel_internal_bridged_channel(original) && (ast_channel_internal_bridged_channel(original) != ast_bridged_channel(original)) && (ast_channel_internal_bridged_channel(ast_channel_internal_bridged_channel(original)) != original))
		{
			final_orig = ast_channel_internal_bridged_channel(original);
		}
		if (ast_channel_internal_bridged_channel(clonechan) && (ast_channel_internal_bridged_channel(clonechan) != ast_bridged_channel(clonechan)) && (ast_channel_internal_bridged_channel(ast_channel_internal_bridged_channel(clonechan)) != clonechan))
		{
			final_clone = ast_channel_internal_bridged_channel(clonechan);
		}
		if (ast_channel_tech(final_clone)->get_base_channel && (base = ast_channel_tech(final_clone)->get_base_channel(final_clone)))
		{
			final_clone = base;
		}
...
void ast_channel_set_caller_event(struct ast_channel *chan, const struct ast_party_caller *caller, const struct ast_set_party_caller *update)
{
	...
	ast_party_caller_set(ast_channel_caller(chan), caller, update);
	if (S_COR(ast_channel_caller(chan)->id.number.valid, ast_channel_caller(chan)->id.number.str, NULL) != pre_set_number || S_COR(ast_channel_caller(chan)->id.name.valid, ast_channel_caller(chan)->id.name.str, NULL) != pre_set_name)
	{
												 ...
												 struct ast_channel **rc)
{
	...
	for (;;)
	{
		...
		if (!who)
		{
			...
			if ((ast_channel_softhangup_internal_flag(c0) | ast_channel_softhangup_internal_flag(c1)) & AST_SOFTHANGUP_UNBRIDGE)
			{ /* Bit operators are intentional. */
...
struct ast_channel __attribute__((format(printf, 10, 11))) * ast_channel_alloc(int needqueue, int state, const char *cid_num,
																			   const char *cid_name, const char *acctcode,
																			   const char *exten, const char *context,
																			   const char *linkedid, const int amaflag,
																			   const char *name_fmt, ...);
