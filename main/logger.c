...
<VITEL> static char vitel_log_name[256] = VITELOG;
</ VITEL> static char exec_after_rotate[256] = "";

static int filesize_reload_needed;
static unsigned int global_logmask = 0xFFFF;
static int queuelog_init;
<VITEL> static int vitelog_init;
</ VITEL> static int logger_initialized;
...
static struct
{
	...
	<VITEL> unsigned int vitel_log : 1;
	</ VITEL> unsigned int queue_log_to_file : 1;
...
<VITEL> static void logger_vitel_init(void);
</ VITEL>
...
static void init_logger_chain(int locked, const char *altconf)
{
	...
	<VITEL> if ((s = ast_variable_retrieve(cfg, "general", "vitel_log")))
	{
		logfiles.vitel_log = ast_true(s);
	}
	</ VITEL> if ((s = ast_variable_retrieve(cfg, "general", "queue_log_to_file")))
	{
...
<VITEL> void ast_vitel_log(const char *callid, const char *funcName, const char *src, const char *dst, const char *exten, const char *event, const char *data, const char *fmt, ...)
{
	va_list ap;
	char vlog_msg[8192];
	int vlog_len;
...
<VITEL> static int logger_vitel_restart(int vitel_rotate)
{
	int res = 0;
	...
	static int reload_logger(int rotate, const char *altconf)
{
	...
	AST_RWLIST_TRAVERSE(&logchannels, f, list)
	{
		...
		{
			int rotate_this = 0;
			if (ftello(f->fileptr) > 0x40000000)
			{ /* Arbitrarily, 1 GB */
	...
	<VITEL> ast_unload_realtime("vitel_log");
	</ VITEL> if (logfiles.queue_log)
	{
...
void __ast_verbose_ap(const char *file, int line, const char *func, int level, struct ast_callid *callid, const char *fmt, va_list ap)
{
	struct ast_str *buf = NULL;
	int res = 0;
	const char *prefix = level >= 4 ? VERBOSE_PREFIX_4 : level == 3 ? VERBOSE_PREFIX_3
													 : level == 2	? VERBOSE_PREFIX_2
													 : level == 1	? VERBOSE_PREFIX_1
																	: "";
